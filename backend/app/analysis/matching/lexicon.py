...
# Turkish final-consonant voicing alternations (common in OCR errors and informal spelling)
_TURKISH_CONSONANT_PAIRS = [("t", "d"), ("p", "b"), ("k", "g"), ("c", "ç"), ("ç", "c")]


def _turkish_consonant_variants(text: str) -> List[str]:
    """Generate variants by swapping final consonants of each word (Turkish consonant mutation)."""
    parts = text.split()
    if not parts:
        return []
    extra = []
    for i, word in enumerate(parts):
        if len(word) < 3:
            continue
        for orig, alt in _TURKISH_CONSONANT_PAIRS:
            if word.endswith(orig):
                new_word = word[:-len(orig)] + alt
                new_parts = parts[:i] + [new_word] + parts[i+1:]
                extra.append(" ".join(new_parts))
            elif word.endswith(alt):
                new_word = word[:-len(alt)] + orig
                new_parts = parts[:i] + [new_word] + parts[i+1:]
                extra.append(" ".join(new_parts))
    return extra
            ...
            if parts:
                ...
                if stripped and stripped != parts[-1]:
                    ...
                    if alt and alt not in variants:
                        variants.append(alt)
    # Add Turkish consonant alternation variants
    base_variants = list(variants)
    for base in base_variants:
        for cv in _turkish_consonant_variants(base):
